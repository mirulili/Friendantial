...
class AbstractLLMClient(ABC):
    ...
    """
    @abstractmethod
    async def generate_chat_completion(self, messages: List[Dict[str, str]], model: str) -> str:
        """
...
class GeminiChatClient(AbstractLLMClient):
    ...
    async def generate_chat_completion(self, messages: List[Dict[str, str]], model: str) -> str:
        # Gemini는 model 객체를 먼저 생성해야 합니다.
        if self._model is None or self._model.model_name != model:
            self._model = genai.GenerativeModel(model)

        # OpenAI의 'system' 역할을 Gemini 형식에 맞게 변환합니다.
        # 여기서는 간단히 첫 메시지를 시스템 프롬프트로 간주합니다.
        # 실제로는 더 정교한 변환 로직이 필요할 수 있습니다.
