  ...
  <stock code='{item.code}' name='{item.name}'>
    ...
    {news_xml}
        ...
        if strategy == "day_trader":
            perspective_instruction = (
                "<perspective>단기 트레이더의 관점에서 분석합니다. 특히 '5일선 이탈', "
                "'거래량 급등'과 같은 단기 신호와 최신 뉴스의 영향을 중요하게 다룹니다.</perspective>"
            )
        elif strategy == "long_term":
            perspective_instruction = (
                "<perspective>장기 투자자의 관점에서 분석합니다. 장기 모멘텀(m60)과 "
                "'장기 추세 이탈' 같은 안정성을 중심으로 설명합니다.</perspective>"
            )

        # 4. 최종 프롬프트 조합 (f-string 사용)
        format_instruction = (
            "이모지를 사용하여 친근함을 더합니다." if persona_name == "friend" else ""
        )
        user_prompt = f"""
<data>
...
<instructions>
  ...
  <format>각 종목을 제목으로 하고, 핵심 내용을 1~2문단으로 요약하여 설명합니다. {format_instruction}</format>
  {perspective_instruction}
</instructions>
"""

        # 캐싱 로직이 내장된 generate_text_with_persona 함수를 호출
    ...
    try:
        ...
        if True: # Indentation placeholder
            try:
                ...
                logging.warning(
                    f"종목 정보 조회 실패 ({stock_code}): {e.detail}. 코드로 계속 진행합니다."
                )
            except Exception as e:
        ...
        if df is None or df.empty:
            raise HTTPException(
                status_code=404,
                detail=f"{stock_code}에 대한 시세 데이터를 찾을 수 없습니다.",
            )
...
<instructions>
  ...
  <format>기술적 분석, 뉴스 분석, 그리고 최종 결론 순서로 문단을 나누어 설명합니다. {format_instruction}</format>
  <perspective>모멘텀(상승/하락 추세), 변동성(안정성), 그리고 최신 뉴스가 주가에 미치는 영향을
  종합적으로 연결하여 하나의 흥미로운 스토리로 풀어냅니다.</perspective>
</instructions>
"""
